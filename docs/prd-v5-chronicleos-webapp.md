...
7. **Efeitos de Rolagem Reativos Automáticos (Horror e Comédia)**: Disparo reativo automático de animações e áudios de acordo com o resultado da parada de dados. O Horror da Besta (visão de túnel vermelha e batimentos cardíacos) ativa-se em Falha Bestial/Sucesso Crítico Bagunçado. O Alívio Cômico (mascote clássico do Toasty fixo e áudio clássico) ativa-se em Falha Pura (0 sucessos) ou Massacre (4+ sucessos).
8. **Blindagem do Escudo do Mestre no Chat e Websocket**: Proteção estrita das rolagens confidenciais do Narrador com máscara completa de dados no backend (limpando as chaves `isBestialFailure` e `isMessianic` no payload público dos jogadores) e aplicação de guard clauses no frontend para impedir que os efeitos rodem fora da tela do Narrador. O card no feed do Narrador recebe estilização dourada com borda tracejada e a badge `Escudo do Mestre 🛡️`.