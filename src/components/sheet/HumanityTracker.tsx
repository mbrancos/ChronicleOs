...
export default function HumanityTracker({
  ...
  return (
    <div className={`space-y-3 p-3 rounded-sm border transition-all duration-300 ${
      ...
      {/* ===== CONTROLES SEPARADOS MINIMALISTAS ===== */}
      {!disabled && (
        <div className="flex flex-wrap items-center justify-between gap-2 pt-0.5">
          {/* Controle de Humanidade */}
          <div className="flex items-center gap-1.5">
            <span className="text-[10px] font-data uppercase tracking-wider text-text-muted font-bold">Humanidade</span>
            <div className="flex items-center gap-1 bg-bg-main/60 border border-white/10 rounded-xs px-1.5 py-0.5">
              <button
                onClick={() => adjustHumanity(-1)}
                disabled={isUpdatingHumanity || isWassail}
                className="w-4.5 h-4.5 bg-white/5 border border-white/10 hover:border-gold-accent/50 hover:bg-white/10 text-[9px] font-bold text-text-muted hover:text-gold-accent flex items-center justify-center rounded-xs transition-colors cursor-pointer select-none disabled:opacity-30 disabled:cursor-not-allowed"
                title="Reduzir Humanidade"
              >
                −
              </button>
              <span className="text-xs font-data font-bold text-gold-accent w-3 text-center tabular-nums">{humanity}</span>
              <button
                onClick={() => adjustHumanity(1)}
          ...
          {/* Controle de Máculas */}
          <div className="flex items-center gap-1.5">
            <span className="text-[10px] font-data uppercase tracking-wider text-text-muted font-bold">Máculas</span>
            <div className="flex items-center gap-1 bg-bg-main/60 border border-white/10 rounded-xs px-1.5 py-0.5">
              <button
                onClick={handleRemoveStain}
                disabled={isUpdatingStains || stains <= 0 || isWassail}
                className="w-4.5 h-4.5 bg-white/5 border border-white/10 hover:border-hunger-red/50 hover:bg-white/10 text-[9px] font-bold text-text-muted hover:text-hunger-red flex items-center justify-center rounded-xs transition-colors cursor-pointer select-none disabled:opacity-30 disabled:cursor-not-allowed"
                title="Remover Mácula"
              >
                −
              </button>
              <span className={`text-xs font-data font-bold w-3 text-center tabular-nums ${stains > 0 ? "text-hunger-red" : "text-text-dim"}`}>{stains}</span>
              <button
